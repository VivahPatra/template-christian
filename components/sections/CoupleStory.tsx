...
export default function CoupleStory() {
  ...
  return (
    <section id="story" className="relative overflow-hidden py-28 px-6 font-serif" style={{ background: 'var(--color-surface2)', color: '#2a2420' }}>
      <FlowerOverlay />
              ...
              return (
                ...
                >
                  ...
                  <div className={`${isLeft ? 'md:text-right md:pr-12' : 'md:order-2 md:pl-12'}`}>
                    ...
                    {/* Text */}
                    <EditableText field="date" arrayField="coupleStory" index={idx} tag="p" className="font-sans text-xs tracking-widest uppercase mb-2" style={{ color: 'var(--color-accent)', opacity: 0.65 }}>
                      {milestone.date}
                    </EditableText>
                    <EditableText field="title" arrayField="coupleStory" index={idx} tag="h3" className="font-display text-2xl mb-2 glow-text" style={{ color: 'var(--color-accent)' }}>
                      {milestone.title}
                    </EditableText>
                    <EditableText field="description" arrayField="coupleStory" index={idx} tag="p" className="font-serif text-sm leading-relaxed" style={{ color: 'var(--color-muted)' }} multiline>
                  ...
                  <div className={`hidden md:flex items-center justify-center ${isLeft ? 'order-2' : 'order-1'}`}>
                    <div className="relative flex items-center justify-center">
                      ...
                      />
                      <div className="w-12 h-12 rounded-full flex items-center justify-center text-2xl" style={{ background: 'var(--color-surface3)', border: '1px solid var(--color-border-strong)' }}>
                        {milestone.icon}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="md:hidden flex items-center gap-3 -mt-4">
                    <div className="w-10 h-10 rounded-full flex items-center justify-center text-lg flex-shrink-0" style={{ background: 'var(--color-surface3)', border: '1px solid var(--color-border-strong)' }}>
                      {milestone.icon}
                    </div>
